...
|-------------------------------|------------------------------------------------------------------------------------------|---------------------|
| main                        | برنچ اصلی تولید (Production) - نسخه پایدار و نهایی پروژه                                 | فعال               |
| gh-pages                    | برنچ اختصاصی برای GitHub Pages - استقرار خودکار سایت استاتیک                              | فعال (Deployment)   |
| feature/conference-ui       | توسعه رابط کاربری صفحه کنفرانس                                                          | مرج شده به main     |
| feature/french-lang         | افزودن پشتیبانی زبان فرانسوی                                                | مرج شده به main     |
| hotfix/github-pages-config  | رفع مشکل تنظیمات GitHub Pages و اطمینان از لود صحیح سایت                                   | مرج شده به main     |
...
```
https://se-lab-ashkan-ayeen.github.io/exp1/‍‍‍
``` 
...
در گیت، هر commit در واقع یک **snapshot** از فایل‌ها (از درخت فایل‌ها) است و نه مجموعه‌ای از diffs خط‌به‌خط بین نسخه‌ها (اگرچه دیتاها به صورت اشیاء و دلتاها در زیر سطح ذخیره می‌شوند ولی مدل مفهومی گیت این‌طور است که commit یک عکس فوری از پروژه است). هنگامی که commit می‌گیرید، گیت یک snapshot از وضعیت index می‌سازد (هر فایلی که در stage است وارد snapshot می‌شود). هر commit به snapshot خود (و به والد/والدها در صورت merge) ارجاع می‌دهد.
...
- **Local repository**
  ...
  - چیزی است که روی ماشین شما قرار دارد (دائما با .git و working directory و index). شما می‌توانید بدون اینترنت commit، branch، revert و خیلی کارهای دیگر انجام بدهی.
  - تاریخچه‌ی محلی ممکن است شامل commitهایی باشد که هنوز به ریموت نرفته‌اند (un-pushed commits).

- **Remote repository**
  - نسخه‌ای از مخزن که در سرور یا سرویس میزبانی (مثل GitHub/GitLab/Bitbucket یا سرور داخلی) قرار دارد.
  - ریموت به‌صورت URL ای ذخیره می‌شود (origin معمولا نام پیش‌فرض).
  - دستورات push و pull برای همگام‌سازی بین local و remote استفاده می‌شوند.

**تفاوت‌های کلیدی:**

- دسترسی و همگام‌سازی: remote برای اشتراک‌گذاری با دیگران و نگهداری یک منبع مرکزی به کار می‌رود.
- قواعد و مجوزها: remote ممکن است کنترل دسترسی، CI، و قوانین merge داشته باشد.
- تاریخچه محلی ممکن است شامل چیزهایی باشد که در remote نیست و برعکس (تا زمانی که sync نکرده باشید).

برای مثال:

```
git push origin main   # فرستادن commitهای محلی به ریموت
git fetch origin       # دریافت تغییرات از ریموت بدون ادغام
```
